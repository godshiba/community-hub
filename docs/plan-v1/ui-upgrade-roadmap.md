...
| 5 | Charts & editors | Complete | 5 / 5 |
| 6 | Motion pass | Not started | 0 / 8 |
| 7 | Polish & QA | Not started | 0 / 10 |

**Overall:** 86 / 112 tasks complete.
...
- [x] **4.2** Moderation — FilterBar, virtualized ListRow list, inspector with MemberSummary, detail takeover, bulk-select, right-click menu.
- [x] **4.3** Events — List/Calendar toggle, EventDetail inspector, calendar grid with event pills.
- [x] **4.4** Scheduler — Queue/History toggle, PostPreview inspector, drag-reorder queue.
- [x] **4.5** Agent — Start/Pause toggle, ActionFeed virtualized list, ActionDetail inspector, conversation/knowledge SegmentedControl.
- [x] **4.6** Reports — Generate/History toggle, GeneratorOptions inspector, ReportPreview, progress bar while generating.
- [x] **4.7** Settings — left-rail + right-detail layout (not a sheet), dirty-state indicators per section, Save buttons, unsaved-changes confirmation.
...
- [x] **5.4** Create `src/renderer/components/editor/` with TipTap theme: body, toolbar, bubble menu, headings, code blocks, blockquote.
- [x] **5.5** Apply TipTap theme to Scheduler PostEditor and any other editor usage.
...
- **5.4 (2026-04-26):** Installed TipTap v3.22.4 + tiptap-markdown 0.9.0. New `src/renderer/components/editor/` houses `Editor.tsx` (StarterKit + Link + Placeholder + Markdown), `EditorToolbar.tsx` (Phosphor icon-only toolbar with Bold/Italic/Strike/inline Code, list/blockquote/code-block, link), and `editor.css` (15px SF Pro body, blockquote with accent left-border, code blocks on `--color-surface-input` with SF Mono 13px, accent links, accent-fill ::selection). Markdown is the source of truth in/out via `editor.storage.markdown.getMarkdown()` — preserves the existing `string` payload contract for posts. Bubble menu deferred — not currently used by the only consumer (PostEditor).
- **5.5 (2026-04-26):** PostEditor's plain `TextArea` for content swapped for the new `Editor`. Markdown text flows back into the existing `payload.content: string` so the scheduler IPC, repository, and post-sender are untouched. Discord renders markdown natively; Telegram still shows raw markdown (existing behavior — no `parse_mode` change). Bundle grew ~1.1MB un-gzipped (TipTap is the bulk); acceptable for an Electron desktop app and consistent with the spec calling TipTap "kept" in the dependency baseline.
...
- **4.4 (2026-04-26):** SchedulerPanel rewritten with `usePanelToolbar` (Queue/History SegmentedControl + primary New Post button + ⌘N tooltip). Inspector enabled and renders new `PostPreview` (per-platform mock cards for queue selection, success/error meta for history). PostQueue + PostHistory migrated to `Surface` + `ListRow` + status/platform `Pill`s; per-row `ContextMenu` (Send now / Cancel) and inline icon buttons. PostEditor rebuilt as `Sheet` (md) using `FormRow` + `TextField` / `TextArea` / `Toggle` (per-platform) + `Select` (channel) + draft / schedule / send footer buttons. Shell ⌘N / ⇧⌘N now also dispatch `panel:newPost` / `panel:newEvent` shell events; Scheduler/Events panels listen and open the editor sheet. Drag-reorder for queue is **deferred** — there's no backend `sequence` field on `scheduled_posts` and adding one is out of scope for the UI-only Phase 4 boundary; will be picked up in Phase 7 polish or as a follow-up.
- **4.3 (2026-04-26):** EventsPanel rewritten with `usePanelToolbar` (List/Calendar SegmentedControl + New Event primary button + ⇧⌘N shortcut tooltip). Inspector enabled and renders `EventDetail` (selected event or `EmptyState`). EventList migrated to Surface + ListRow + Pill (status & platform). EventCalendar uses Phosphor caret icons in Surface header; today highlight uses `--color-accent` outline; event pills use `--color-accent-fill`. EventForm rebuilt as `Sheet` width=md with `FormRow` + `TextField` / `TextArea` / `Select` / `NumberField` / `Checkbox`. All `GlassCard`/`GlassModal` removed from events panel.
- **4.2 (2026-04-26):** Members tab fully migrated to ui-native primitives. New `MembersList.tsx` uses `@tanstack/react-virtual` virtualizer with sticky header, `TextField`/`Select` filter bar, `Checkbox` bulk-select column, per-row `ContextMenu` (Open/Warn/Ban/Unban/Copy ID). New `MemberSummary.tsx` renders inspector content (avatar + status pills + KV grid + recent warnings). Detail takeover opens existing `MemberDetailPanel` inside a `Sheet width="lg"`. `BulkActionToolbar` migrated to `Surface` + native `Sheet` confirms. `ModerationPanel` registers SegmentedControl tabs in toolbar; inspector enabled only on Members tab per spec. Content/Audit/Spam tabs render unchanged inside the new shell — full inspector wiring for those tabs is deferred (current views have no per-row selection model). Wires `panel:focusSearch`, `panel:selectAll`, `panel:deselectAll`, `panel:openDetail` shell events.
